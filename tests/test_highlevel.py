...
from ropt.highlevel._objective import adapt_objective
from ropt.workflow.evaluators import (
    EvaluationFunctionContext,
)
from ropt.workflow.event_handlers import HistoryHandler
...
def test_top_level_lazy_reexport() -> None:
    assert ropt.optimize is optimize
    assert ropt.optimize_many is optimize_many
    assert ropt.evaluate is evaluate
    assert ropt.evaluate_many is evaluate_many
    assert ropt.OptimizeResult is OptimizeResult
    assert ropt.EvaluateResult is EvaluateResult
    assert "optimize" in dir(ropt)


def test_top_level_unknown_attribute_raises() -> None:
    with pytest.raises(AttributeError, match="has no attribute"):
        _ = ropt.does_not_exist
