...
class VariablesConfig(BaseModel):
    ...
    variables. It is used in an [`EnOptContext`][ropt.context.EnOptContext] object
    ...
    **Variable perturbations**
    ...
    objects in the configuration object used by an optimization step.
    [`EnOptContext`][ropt.context.EnOptContext] object defines the available
    samplers in its `samplers` field. The `samplers` field of the
    `VariablesConfig` object specifies, for each variable, the the sampler to
    use by its index into the `samplers` tuple. A random number generator is
    created to support samplers that require random numbers.
