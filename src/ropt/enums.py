...
class EventType(IntEnum):
    ...
    and callbacks can be connected to these events using the
    [`add_observer`][ropt.optimization.EnsembleOptimizer.add_observer] method.
    When triggered by an event, the callbacks receive an
    [OptimizationEvent][ropt.events.OptimizationEvent] object. This object
    contains at least the type of the event (a value of this enumeration) and
    the current configuration of the step in the plan that is executing.
    Additionally, depending on the event type, a tuple of result objects or an
    exit code may be present. Refer to the documentation of the individual event
    types for details.
    """
