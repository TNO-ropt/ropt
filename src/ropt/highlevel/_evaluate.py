...
from ropt.workflow.compute_steps import EvaluationStep
...
def evaluate(
    ...
    Use [`evaluate_many`][ropt.highlevel.evaluate_many] to evaluate several
    vectors at once. See [High-Level API](../usage/highlevel.md) for a
    walkthrough.
    ...
    Returns:
        An [`EvaluateResult`][ropt.highlevel.EvaluateResult] for the vector.
...
def evaluate_many(
    ...
    the same order. See [High-Level API](../usage/highlevel.md) for a
    walkthrough.
    ...
    Returns:
        One [`EvaluateResult`][ropt.highlevel.EvaluateResult] per input vector.
