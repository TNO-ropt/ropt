...
@dataclass
class Results:
    """The `Results` class serves as a base class for storing results.

    This class contains the following generic information:

    1. The evaluation ID, a unique integer value that is incremented as the
    ...
    def to_dataframe(
        ...
        general such a field is another data frame object with multiple fields.
        By default, these are all exported as columns in the pandas data frame,
        but a sub-set can be selected using the `select` argument.
    ...
    def to_dataset(
        ...
        *,
        ...
        general, such a field is another data frame object with multiple fields.
        By default, these are all exported as data arrays in the xarray dataset,
