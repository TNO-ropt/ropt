...
State: TypeAlias = Literal["pending", "running", "done", "error"]


class PollingServer(ServerBase[Task[R, TR]]):
    """A base class for servers that employ polling."""

    def __init__(
        self, *, workers: int = 1, maxsize: int = 0, interval: float = 1
    ) -> None:
        """Initialize the server.
        ...
        self._tasks: dict[Task[R, TR], State] = {}
        ...
        while self._running.is_set():
            ...
            for task, state in self._tasks.items():
                if state in {"done", "error"}:
                    task.put_result(self.get_result(task))
                    remove.append(task)
            for task in remove:
                self._tasks.pop(task)
            if len(self._tasks) < self._workers:
                task = await self._task_queue.get()
                self._tasks[task] = "pending"
                self.submit(task)
                self._task_queue.task_done()
    ...
    def get_result(self, task: Task[R, TR]) -> R:
        ...
        Args:
            task: The task to get the result of.
