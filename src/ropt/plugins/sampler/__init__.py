...
* **Plugin Interface:** Sampler plugins must inherit from the
  [`SamplerPlugin`][ropt.plugins.sampler.SamplerPlugin] base class.
  This class acts as a factory, defining a `create` method to instantiate
  sampler objects.
* **Sampler Implementation:** The actual sampling logic resides in classes that
  inherit from the [`Sampler`][ropt.sampler.Sampler] abstract base class. These
  classes are initialized with the optimization configuration
  ([`EnOptContext`][ropt.context.EnOptContext]), the index of the specific sampler
  configuration to use (`sampler_index`), an optional variable mask (`mask`),
  and a random number generator (`rng`). Samples are generated by calling the
