...
`variable_transforms` is the exception: nothing selects from it. It is an
ordered [chain](variable_transforms.md) applied to every variable, in order
towards the optimizer domain and in reverse coming back.
...
`realization_filters`, and `variable_transforms`. You can mix these forms
...
configured in the
[`samplers`](#function_estimators-realization_filters-samplers-variable_transforms)
...
### `function_estimators`, `realization_filters`, `samplers`, `variable_transforms`
...
key from the sections that use them (see [Sharing optimizer components by
key](#sharing-optimizer-components-by-key) above). `variable_transforms` is
not: it is an ordered [chain](variable_transforms.md) applied to every variable.
...
[Transforms](variable_transforms.md) modify variables as they pass between the user's
domain and the optimizer's domain:
...
It has a `method` field, an `options` dict (default: `{}`), and an optional
boolean `mask` (default: `None`) selecting the variables the transform applies
to. `None` means all of them. The transform decides how to honour its mask; see
[Transforms](variable_transforms.md#restricting-a-transform-to-a-subset).
