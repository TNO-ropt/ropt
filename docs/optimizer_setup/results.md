...
- **`evaluation_point`**: the variables this result was produced at, in the user
  domain. It records the point the evaluation function was actually called with,
  and is never transformed, so it identifies the result no matter which domain
  the other fields are in.
