...
[disp, maxiter, keep_feasible](https://docs.scipy.org/doc/scipy/reference/generated/scipy.optimize.minimize.html#scipy.optimize.minimize)
...
|[L-BFGS-B](https://docs.scipy.org/doc/scipy/reference/optimize.minimize-lbfgsb.html)^2^|*disp*, maxcor, ftol, gtol, eps, maxfun, iprint, maxls, finite_diff_rel_step|
|[TNC](https://docs.scipy.org/doc/scipy/reference/optimize.minimize-tnc.html)^3^|maxfun, eps, scale, offset, maxCGit, eta, stepmx, accuracy, minfev, ftol, xtol, gtol, rescale, finite_diff_rel_step, ~~maxiter~~|
...
1. The `keep_feasible` option is used to maintain feasibility with respect to bound, linear and non-linear constraints, by passing it to the constraint handling code of the underlying SciPy optimizer. Some algorithms may choose to ignore this option.
2. Options in *italics* override a common option with a different type or behavior.
3. Options with ~~strikethrough~~ indicate a common option that is not supported.
