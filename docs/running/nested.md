...
!!! tip "This page also covers four things that are not nesting"
    ...
    into one program small enough to read end to end: choosing an executor per
...
around it — which
variables each layer owns, which executor each layer evaluates on, and how to
get the results out.
...
flowchart TB
    subgraph proc["your process"]
        ...
        subgraph touter["outer ThreadExecutor — 2 threads"]
            direction TB
    ...
    subgraph pinner["inner ProcessExecutor — 2 workers"]
        direction TB
        w1["worker"]
        w2["worker"]
    end
...
process, and dotted arrows carry results back to the handler. Both executors
are drawn: the outer one is inside your process, the inner one is not. The
...
Each layer evaluates on its own executor, and that is a requirement rather than
a preference:
...
The outer executor is a **thread** executor. Outer evaluations therefore stay
inside this process, where the inner executor and the shared handler are live
objects; on a process executor they would arrive as copies, which `ropt`
refuses. The inner executor is a **process** executor, which is where the real
work goes.

That is also where a cluster belongs. Swapping the inner executor for an
[`HPCExecutor`][ropt.simple.HPCExecutor] is the whole change — the inner
evaluations become cluster jobs, and the layer above is untouched:

```python
inner_executor = HPCExecutor(workers=50, workdir="/scratch/my-run")
```

Sending whole *optimizations* to the cluster, by putting the **outer**
evaluations on processes or HPC, is possible but gives something different. Each
outer evaluation is then copied into a job, and neither the inner executor nor
the shared handler can travel with it: the job has to build its own executor and
collect its own results, and return them as data for you to combine. Use that
when the outer evaluations are genuinely independent; use the shape above when
you want one executor and one table across all of them.
...
each feeding the inner executor one bundle, so both workers are busy and each
batch costs one transfer instead of five. See
[How a batch is split across workers](parallel.md#how-many-workers).

Handing the inner run the executor it is already running on would deadlock, and
`ropt` refuses it rather than hanging — see
[Executors inside an evaluation](parallel.md#executors-inside-an-evaluation) for
why.
...
an ordinary dictionary, created beside the executors and passed to the function
through `partial`.
...
in this process. On a `ProcessExecutor` or an `HPCExecutor` the function is
