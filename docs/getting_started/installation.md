...
import, which on a cluster means installed on the compute nodes. Without
`cloudpickle`, local and cluster jobs therefore accept only an evaluation
function that already lives in such a module.
