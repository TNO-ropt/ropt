...
open session and its pool. Any of ``config``/``x0``/``objective`` may be a single
value (broadcast to every run) or a per-run sequence; here a matrix of start
vectors sets the number of runs while the config and objective are broadcast.
"""
