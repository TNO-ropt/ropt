...
block, so the same `optimize` call submits its evaluations as cluster jobs. This
requires the ``ropt[hpc]`` extra and a reachable cluster; the script is
illustrative and is not run as part of the test suite.
"""
